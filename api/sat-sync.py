...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            # ─────────────────────────────────────────────────────────
            # ACCIÓN: DESCARGA DE DOCUMENTOS (PDF/XML) O RETENCIONES
            # ─────────────────────────────────────────────────────────
            if action == 'retenciones_pdf':
                try:
                    num = params.get('numero')
                    tipo_ret = params.get('tipoRetencion', 'IVA')
                    d_start = datetime.datetime.strptime(date_start_str, "%Y-%m-%d")
                    d_end = datetime.datetime.strptime(date_end_str, "%Y-%m-%d")
                    
                    pdf_bin = sat.get_retencion_pdf(num, d_start, d_end, tipo_ret)
                    if not pdf_bin: raise Exception("No se pudo obtener el PDF de la retención")
            ...
            def process_fast(inv_raw):
                try:
                    ...
                    return {
                        ...
                        'estado': 'A' if inv_raw.get('anulado') == 'I' else 'V', # I=anulado, V=vigente
            ...
            if supa_url and supa_key and invoices:
                ...
                for inv in invoices:
                    ...
                    rec = {
                        ...
                        'fel_uuid': inv['uuid'], 'status': 'annulled' if inv['estado'] == 'A' else 'paid',
