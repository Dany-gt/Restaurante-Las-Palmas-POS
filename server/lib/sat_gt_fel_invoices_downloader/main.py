...
class SatFelDownloader:
    ...
    def _get_invoices_headers(self, filter: SATFELFilters):
        logging.info("CALL URL GET FEL (MULTIPAGE)")
        ...
        if not cookie:
            # Let's see what cookies we actually have!
        ...
        while True:
            ...
            logging.info(f"Querying invoices page {page}...")
            url = (
            ...
            json_res = r.json()
            data = json_res.get("detalle", {}).get("data", [])
            ...
            logging.info(f"Page {page} ok, total so far: {len(all_invoices)}")
            
            # Si el total devuelto en la página es menor al límite, es la última página
