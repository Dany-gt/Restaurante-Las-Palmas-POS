...
### 🚨 STRICT TASK FOCUS PROTOCOL (MANDATORY)
1. **No Unsolicited Changes**: You must ONLY modify files and write code that directly answers the user's explicit request. Do NOT touch unrelated files, add speculative features, refactor code unless requested, or perform cleanup that wasn't asked for.
2. **No Double/Duplicate Work**: Do not repeat operations that are already completed. Double check state and git history before editing.
3. **No Off-Topic Explanations**: Keep explanations direct and strictly focused on the requested task. Do not mention system internals, agent roles, or unrelated files.
4. **Minimal Autonomy**: Always verify details with the user before executing potentially invasive changes.

### 📐 LAYOUT & COMPONENT CHANGES PROTOCOL (MANDATORY)
When a change to a layout, table, modal, or component with specific dimensions is requested:
1. **Code Review**: Examine the relevant components and styles first.
2. **Impact Evaluation**: Evaluate the layout impact and explain it clearly to the user before editing.
3. **Preservation**: Be extremely careful not to write, modify, or delete any code that is unrelated to the request.
4. **Verification**: Run tests or verify the UI/logic to ensure it is delivered fully functional and error-free.
