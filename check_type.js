...
async function checkColType() {
    const { data, error } = await supabase.from('orders').select('discount_amount').limit(10);
    if (data) {
        console.log("discount_amount types:", data.map(d => typeof d.discount_amount + " | val: " + d.discount_amount));
    }
}
