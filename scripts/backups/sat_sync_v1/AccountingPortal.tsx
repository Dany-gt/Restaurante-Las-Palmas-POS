...
export const AccountingPortal: React.FC = () => {
    ...
    const handleSatSync = async () => {
        ...
        try {
            ...
            if (!settings?.sat_username || !settings?.sat_password) {
                alert('⚠️ No hay credenciales SAT configuradas.');
    ...
    const renderTab = () => {
        ...
        switch (activeTab) {
            ...
            case 'estados':   return <TabEstadosFinancieros accentColor={TABS[5].accent} />;
            case 'calendario':return <TabCalendarioFiscal accentColor={TABS[6].accent} onNavigate={setActiveTab} />;
            case 'auditoria': return <TabAuditoriaSAT accentColor={TABS[7].accent} />;
            default:          return null;
        }
    };
