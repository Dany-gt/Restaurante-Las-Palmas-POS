        ...
        def process_invoice(inv_raw):
            try:
                ...
                return {
                    ...
                    'fecha': fecha,
                    'total': total,
                    'iva': float(inv_raw.get('montoIva', 0) or 0),
                    'neto': total - float(inv_raw.get('montoIva', 0) or 0),
                    'serie': inv_raw.get('serie'),
                    'numero': inv_raw.get('numero'),
                    'uuid': inv_raw.get('numeroUuid'),
                    'estado': inv_raw.get('estadoDte', 'V'),
                    'tipo_dte': inv_raw.get('tipoDte'),
                    'items': items
                }
            except Exception:
