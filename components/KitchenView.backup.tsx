    ...
    if (loading) return (
        <div className="h-full flex items-center justify-center bg-[#0f1115]">
            <div className="flex flex-col items-center gap-4">
                ...
                <span className="text-[10px] font-semibold uppercase tracking-[0.4em] text-gray-500">Sincronizando Cocina...</span>
            </div>
        </div>
    );
        ...
        <div className="h-full flex flex-col bg-[#0a0c10] overflow-hidden">
            ...
            <header className="p-8 border-b border-white/5 bg-[#0f1115] flex justify-between items-center ">
                <div className="flex items-center gap-6">
                    ...
                    <div>
                        <h1 className="text-3xl font-semibold tracking-tighter uppercase leading-none">Kitchen Display System</h1>
                        <div className="flex items-center gap-3 mt-1">
                            <span className="text-[10px] font-semibold uppercase tracking-widest text-emerald-400">Restaurante Las Palmas</span>
                            <div className="w-1 h-1 rounded-full bg-white/20"></div>
                            <span className="text-[10px] font-semibold uppercase tracking-widest text-gray-500">{orders.length} Comandas Activas</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex gap-3 items-center">
                    ...
                    {Math.ceil(orders.length / ordersPerPage) > 1 && (
                        <div className="flex items-center gap-2 bg-black/20 p-1 rounded-xl border border-white/5 mr-4">
                            ...
                            <div className="px-3 flex flex-col items-center">
                                <span className="text-[9px] font-semibold tracking-tighter text-indigo-400">PÁGINA {currentPage + 1}</span>
                                <span className="text-[7px] font-medium text-gray-500 uppercase tracking-widest">{Math.ceil(orders.length / ordersPerPage)} TOTAL</span>
                            </div>
                            <button
                    ...
                    <button
                        onClick={unlockAudio}
                        className={`flex items-center gap-3 px-6 py-2.5 rounded-xl text-[9px] font-semibold uppercase tracking-widest transition-all border ${audioUnlocked
                            ...
                            <div key={order.id} className="flex flex-col bg-[#16191f] rounded-[1.5rem] border border-white/5  relative overflow-hidden group h-full">
                                ...
                                <div className={`p-4 border-b border-white/5 flex justify-between items-start transition-colors ${getTimeDiff(order.created_at) > 15 ? 'bg-red-900/10' : 'bg-black/20'
                                    ...
                                    <div>
                                        <div className="flex items-center gap-2 mb-0.5">
                                            ...
                                            <div className="flex flex-col gap-0.5">
                                                {(() => {
                                                    ...
                                                    return (
                                                        <>
                                                            <div className={`px-1.5 py-0.5 rounded-md border text-[9px] font-semibold uppercase tracking-tight flex items-center gap-1.5 ${getWaitColor(waitMinutes)}`}>
                                                                <Clock size={8} /> {formatTimeDiff(waitMinutes)}
                                                            </div>
                                                            {prepMinutes > 0 && (
                                                                <div className="px-1.5 py-0.5 rounded-md border border-indigo-500/30 bg-indigo-500/10 text-indigo-400 text-[9px] font-semibold uppercase tracking-tight flex items-center gap-1.5">
                                        ...
                                        <span className="text-[8px] font-semibold uppercase tracking-widest text-gray-500 truncate block w-40">{order.section} • {order.waiter_name}</span>
                                    </div>
                                            ...
                                            <div className="flex justify-between items-center gap-2">
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-[10px] font-semibold uppercase tracking-tight leading-none truncate">{item.product_name}</span>
                                                        <span className="text-[8px] font-medium text-indigo-400 bg-indigo-400/10 px-1.5 py-0.5 rounded-full">x{item.quantity}</span>
                                                    </div>
                                                    {item.notes && (
                                                        <div className="mt-2 p-2 bg-amber-500/10 border border-amber-500/20 rounded-lg">
                                                            <div className="flex items-center gap-1.5 mb-1">
                                                                <AlertCircle size={10} className="text-amber-500 flex-shrink-0" />
                                                                <span className="text-[8px] font-semibold text-amber-500 uppercase tracking-widest">Indicaciones:</span>
                                                            </div>
                                                            <p className="text-[11px] text-amber-400 font-medium uppercase leading-snug break-words italic">
                                                                "{item.notes}"
                                                            </p>
                                                        </div>
                                                    )}
                                                ...
                                                <div className="flex items-center gap-1 flex-shrink-0">
                                                    {item.status === 'pending' ? (
                                                        <button
                                                            ...
                                                            className="px-2 py-1 bg-indigo-600 hover:bg-indigo-500 text-white rounded-md text-[7px] font-semibold uppercase tracking-tighter transition-all flex items-center gap-1  -600/20"
                                                        >
                                                            <Play size={8} />
                                                        </button>
                                                        ...
                                                        <button
                                                            ...
                                                            className="px-2 py-1 bg-emerald-600 hover:bg-emerald-500 text-white rounded-md text-[7px] font-semibold uppercase tracking-tighter transition-all flex items-center gap-1  -600/20"
                                                        >
                                                            <Check size={9} />
                                                        </button>
                                                    ) : (
                                    ...
                                    {(() => {
                                        ...
                                        if (allReady) {
                                            return (
                                                <button
                                                    ...
                                                    className="w-full py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg font-semibold text-[8px] uppercase tracking-[0.2em]  -600/20 transition-all flex items-center justify-center gap-2"
                                                >
                                        ...
                                        if (anyPending) {
                                            return (
                                                <button
                                                    ...
                                                    className="w-full py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg font-semibold text-[8px] uppercase tracking-[0.2em]  -600/20 transition-all flex items-center justify-center gap-2"
                                                >
                                        ...
                                        return (
                                            <div className="w-full flex items-center justify-center gap-2 text-gray-700 bg-white/5 py-2 rounded-lg border border-white/5">
                                                ...
                                                <span className="text-[7px] font-semibold uppercase tracking-widest">PREPARANDO {selectedStation ? stationName : ''}</span>
                                            </div>
                                        );
                                    })()}
                                </div>
                        ...
                        <div key={`empty-${i}`} className="h-full border border-white/[0.02] rounded-[1.5rem] bg-white/[0.01] flex items-center justify-center">
                            {orders.length === 0 && i === 2 && (
                                <div className="flex flex-col items-center opacity-5">
                                    ...
                                    <span className="text-[10px] font-semibold uppercase tracking-widest text-center">Sin Pedidos</span>
                                </div>
                            )}
                        </div>
                    ))}
                    ...
                    <div className="bg-[#16191f] w-full max-w-4xl rounded-[3rem] border border-white/10  flex flex-col max-h-[90vh] overflow-hidden">
                        <div className="p-8 border-b border-white/5 flex justify-between items-center bg-black/20">
                            <div className="flex items-center gap-6">
                                ...
                                <div>
                                    <h2 className="text-4xl font-semibold uppercase tracking-tighter leading-none">
                                    ...
                                    <div className="flex items-center gap-4 mt-2">
                                        <p className="text-xs font-semibold uppercase text-gray-500 tracking-widest">{selectedOrder.section} • {selectedOrder.waiter_name}</p>
                                        <div className="flex gap-2">
                                            {(() => {
                                                ...
                                                return (
                                                    <>
                                                        <div className={`px-2 py-1 rounded-lg border text-[10px] font-semibold uppercase tracking-widest flex items-center gap-2 ${getWaitColor(waitMinutes)}`}>
                                                            <Clock size={12} /> ESPERA: {formatTimeDiff(waitMinutes)}
                                                        </div>
                                                        {prepMinutes > 0 && (
                                                            <div className="px-2 py-1 rounded-lg border border-indigo-500/30 bg-indigo-500/10 text-indigo-400 text-[10px] font-semibold uppercase tracking-widest flex items-center gap-2">
                                    ...
                                    <div className="flex justify-between items-center gap-6">
                                        <div className="flex-1">
                                            <div className="flex items-baseline gap-4 mb-2">
                                                <span className="text-2xl font-semibold uppercase tracking-tight">{item.product_name}</span>
                                                <span className="text-sm font-medium text-indigo-400 bg-indigo-400/10 px-3 py-1 rounded-full">x{item.quantity}</span>
                                            </div>
                                            {item.notes && (
                                                <div className="mt-3 flex items-start gap-3 bg-black/40 p-4 rounded-2xl border border-white/5">
                                                    ...
                                                    <p className="text-base text-amber-500 font-semibold uppercase leading-relaxed">"{item.notes}"</p>
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        <div className="flex items-center gap-3">
                                            {item.status === 'pending' && (
                                                <button
                                                    onClick={() => updateItemStatus(item.id, 'preparing')}
                                                    className="px-8 py-4 bg-indigo-600 hover:bg-indigo-500 text-white rounded-[2rem] font-semibold uppercase text-xs tracking-widest transition-all flex items-center gap-3"
                                                >
                                            ...
                                            {item.status === 'preparing' && (
                                                <button
                                                    onClick={() => updateItemStatus(item.id, 'ready')}
                                                    className="px-8 py-4 bg-emerald-600 hover:bg-emerald-500 text-white rounded-[2rem] font-semibold uppercase text-xs tracking-widest transition-all flex items-center gap-3"
                                                >
                                                    <Check size={18} /> TERMINAR
                                                </button>
                                            )}
                                            {item.status === 'ready' && (
                                                <div className="flex items-center gap-3 px-8 py-4 bg-emerald-500/20 text-emerald-500 rounded-2xl border border-emerald-500/20 font-semibold uppercase text-xs tracking-widest">
                        ...
                        <div className="p-8 bg-black/40 border-t border-white/5">
                            <button
                                ...
                                className="w-full py-6 bg-white/5 hover:bg-white/10 rounded-[2rem] font-semibold uppercase tracking-[0.3em] transition-all"
                            >
                                Cerrar Detalle
                            </button>
                        </div>
...
const FilterButton = ({ label, active, onClick }: { label: string, active: boolean, onClick: () => void }) => (
    <button
        onClick={onClick}
        className={`px-4 py-2 rounded-xl text-[10px] font-semibold uppercase tracking-widest transition-all ${active ? 'bg-indigo-600 text-white  -600/20' : 'bg-white/5 text-gray-400 hover:bg-white/10 hover:text-white'}`}
    >
        {label}
    </button>
);
