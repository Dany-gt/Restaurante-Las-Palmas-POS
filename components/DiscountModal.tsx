...
export const DiscountModal: React.FC<DiscountModalProps> = ({ isOpen, onClose, subtotal, onApply, currentDiscount, title = "Gestión de Descuentos", itemContext = "Mesa" }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-md p-6 animate-fade-in">
            <div className="w-full max-w-4xl h-auto max-h-[85vh] bg-[#16191f] rounded-[2rem] border border-white/10 shadow-2xl flex overflow-hidden">
                ...
                <div className="flex-1 flex flex-col border-r border-white/5 p-6 md:p-8">
                    ...
                    <div className="flex-1 overflow-y-auto space-y-3 pr-2 scrollbar-hide">
                        {discounts.map(disc => (
                            <button
                                ...
                                className={`w-full flex items-center justify-between p-4 rounded-2xl border transition-all active:scale-95 ${selectedDiscount?.id === disc.id
                                    ? 'bg-rose-600/20 border-rose-500 shadow-lg shadow-rose-600/10'
                            ...
                            >
                                ...
                                <div className="flex items-center gap-3">
                                    <span className="text-lg font-black italic text-rose-400">
                                        {disc.type === 'AMOUNT' ? `-Q${disc.value.toFixed(2)}` : `-${disc.value}%`}
                                    </span>
                                    <div className={`w-5 h-5 rounded-full border flex items-center justify-center transition-all ${selectedDiscount?.id === disc.id ? 'bg-rose-500 border-rose-500' : 'border-white/10'
                                        }`}>
                    ...
                    <div className="mt-6 pt-4 border-t border-white/5">
                        <button
                            ...
                            className="w-full py-3 bg-white/5 hover:bg-red-500/10 text-gray-500 hover:text-red-400 rounded-xl font-black uppercase tracking-widest text-[10px] transition-all flex items-center justify-center gap-2"
                        >
                ...
                <div className="w-[340px] bg-black/20 flex flex-col p-6 md:p-8">
                    <div className="flex justify-between items-center mb-6">
                        <span className="text-[10px] font-black text-rose-400 uppercase tracking-[0.3em]">Detalle Final</span>
                    ...
                    <div className="flex-1 space-y-6 overflow-y-auto">
                        <section>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex justify-between items-center bg-rose-600/10 p-3 rounded-xl border border-rose-500/20">
                                    <span className="text-[10px] font-black text-rose-400 uppercase tracking-widest">Ahorro Aplicado</span>
                        ...
                        <section>
                            <div className="flex justify-between items-center mb-3">
                                ...
                                <span className="text-[8px] font-black text-rose-500 uppercase tracking-widest">Obligatorio</span>
                            </div>
                            <textarea
                                value={reason}
                                onChange={(e) => setReason(e.target.value)}
                                placeholder="Escriba el motivo..."
                                className="w-full h-20 bg-white/5 border border-white/10 rounded-xl p-3 text-xs focus:outline-none focus:border-rose-500 transition-all resize-none"
                            />
                        </section>
                    </div>
                    ...
                    <div className="pt-6 border-t border-white/10 mt-auto">
                        ...
                        <button
                            ...
                            className="w-full py-4 bg-rose-600 hover:bg-rose-500 disabled:bg-[#1a1c23] disabled:text-gray-600 disabled:opacity-50 text-white rounded-xl font-black uppercase tracking-[0.2em] text-[10px] shadow-xl shadow-rose-600/20 transition-all active:scale-95 flex items-center justify-center gap-2"
                        >
