    ...
    return (
        <div className="fixed inset-0 z-[100] flex flex-col bg-[#1a1d26]">
            ...
            <div className="h-14 bg-[#252836] flex items-center px-4 border-b border-white/5">
                ...
                <span className="ml-4 text-[11px] font-medium text-gray-500 uppercase tracking-widest">LAS PALMAS POS</span>
                
                <div className="ml-auto flex items-center gap-6">
                    <div className="text-right">
                        <div className="text-[10px] font-semibold text-white uppercase">{localStorage.getItem('currentUser') ? JSON.parse(localStorage.getItem('currentUser')!).name : 'DANILO PEREZ'}</div>
                        <div className="text-[9px] font-medium text-[#7c7ffb] uppercase">Dividir Cuentas</div>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 flex overflow-hidden">
                ...
                <div className="w-[300px] flex flex-col border-r border-white/5">
                    <div className="h-10 bg-[#2d2e3d] flex items-center justify-center border-b border-white/5">
                        <span className="text-[10px] font-semibold uppercase tracking-widest text-gray-400">Cuentas</span>
                    </div>
                                ...
                                <div className="flex items-center gap-3 overflow-hidden">
                                    ...
                                    {renamingIdx === idx ? (
                                        <input
                                            ...
                                            className="bg-black/20 text-white text-[11px] font-medium uppercase outline-none px-1 w-full"
                                        />
                                    ) : (
                                        <span className="text-[11px] font-semibold uppercase truncate">{acc.name}</span>
                                    )}
                                </div>
                ...
                <div className="flex-1 flex flex-col">
                    <div className="h-10 bg-[#2d2e3d] flex items-center justify-center border-b border-white/5">
                        <span className="text-[10px] font-semibold uppercase tracking-widest text-gray-400">Platillos</span>
                    </div>
                    <div className="flex-1 overflow-y-auto p-6 scrollbar-hide">
                        <div className="flex flex-wrap gap-3">
                            {activeAccount?.items.map((item) => (
                                ...
                                >
                                    <div className="w-6 h-6 bg-black/20 rounded flex items-center justify-center text-[10px] font-medium text-white">
                                        {item.quantity}
                                    </div>
                                    <div className="flex flex-col items-start">
                                        <div className="text-[10px] font-semibold text-gray-200 uppercase leading-tight line-clamp-1">{item.product_name}</div>
                                        <div className="text-[9px] font-medium text-gray-500">Q{(item.price || item.unit_price || 0).toFixed(2)}</div>
                                    </div>
                                </button>
                            ))}
                        </div>
                    ...
                    <div className="h-10 border-t border-white/5 bg-black/10 flex items-center justify-center">
                        <span className="text-[8px] font-medium uppercase tracking-[0.2em] text-gray-600">
            ...
            <div className="h-20 bg-[#252836] border-t border-white/5 flex items-center justify-center gap-10 px-8">
                <button
                    ...
                    className="w-40 h-12 bg-[#3a3b4d] hover:bg-[#45465e] text-[10px] font-semibold uppercase tracking-widest text-gray-400 hover:text-white transition-all rounded"
                >
                    CUENTAS
                </button>
                ...
                <div className="flex gap-2">
                    {accounts.map((acc, idx) => (
                        idx !== activeAccountIdx && selectedItems.size > 0 && (
                            <button
                                ...
                                className="h-12 px-6 bg-[#3a3b4d] hover:bg-[#7c7ffb] text-[10px] font-semibold uppercase tracking-widest text-gray-400 hover:text-white transition-all rounded border border-white/5"
                            >
                    ...
                    {selectedItems.size === 0 && (
                        <button className="h-12 px-10 bg-[#3a3b4d] opacity-50 cursor-not-allowed text-[10px] font-semibold uppercase tracking-widest text-gray-400 rounded">
                            TRASLADAR PLATOS
                        </button>
                    )}
                </div>
                ...
                <button
                    ...
                    className="w-40 h-12 bg-[#7c7ffb] hover:bg-[#6b6edb] text-[10px] font-semibold uppercase tracking-widest text-white transition-all rounded  /20"
                >
                    ACEPTAR
                </button>
            </div>
