            ...
            <div className="bg-[#1e212b] w-full max-w-md rounded-xl border border-white/10  /50 overflow-hidden flex flex-col">
                <div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/5">
                    <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-sm font-semibold uppercase tracking-[0.3em] text-white">MÉTODO DE PROPINA</h3>
                    </div>
                ...
                <div className="p-8 flex flex-col items-center gap-8">
                    <div className="text-center w-full">
                        <span className="text-[10px] font-semibold uppercase tracking-[0.2em] text-gray-500 block mb-2">Monto de Propina</span>
                        <div className="relative group">
                            <div className="flex items-center justify-center gap-2">
                                <span className="text-2xl font-semibold text-white/20">{currency}</span>
                                <input
                                    type="number"
                                    step="0.01"
                                    value={localAmount}
                                    onChange={(e) => setLocalAmount(e.target.value)}
                                    className={`bg-transparent text-5xl font-semibold tabular-nums tracking-tighter border-none outline-none w-48 text-center focus:ring-0 ${isLocked ? 'text-white/80 cursor-pointer' : 'text-white'}`}
                                    autoFocus
                                />
                            </div>
                            <div className={`mt-2 flex items-center justify-center gap-1.5 text-[9px] font-semibold uppercase tracking-widest transition-colors ${isLocked ? 'text-white/40' : 'text-white/20 group-hover:text-white/40'}`}>
                    ...
                    <div className="grid grid-cols-3 gap-3 w-full">
                        ...
                        >
                            ...
                            <span className="text-[10px] font-semibold uppercase tracking-widest text-gray-400 group-hover:text-white">Efectivo</span>
                        </button>
                        ...
                        >
                            ...
                            <span className="text-[10px] font-semibold uppercase tracking-widest text-gray-400 group-hover:text-white">Tarjeta</span>
                        </button>
                        ...
                        >
                            ...
                            <span className="text-[10px] font-semibold uppercase tracking-widest text-gray-400 group-hover:text-white">Otros</span>
                        </button>
                    </div>

                    <button
                        onClick={() => onConfirm('EFECTIVO', 0)}
                        className={`w-full py-4 mt-6 rounded-lg border border-dashed transition-all font-semibold text-[10px] uppercase tracking-[0.2em] ${isLocked ? 'border-white/20 text-white/60 hover:text-white hover:border-white/30 hover:bg-white/5' : 'border-white/10 text-gray-500 hover:text-white hover:border-white/30 hover:bg-white/5'}`}
                    >
                ...
                <div className="p-6 bg-white/5 border-t border-white/5">
                    <button
                        onClick={onClose}
                        className="w-full h-14 rounded-lg border border-white/10 font-semibold uppercase tracking-widest text-gray-400 hover:bg-white/5 transition-all"
                    >
                        CANCELAR
                    </button>
                </div>
