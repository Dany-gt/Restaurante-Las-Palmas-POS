...
export const AdminNotifications: React.FC<AdminNotificationsProps> = ({ currentUser, onViewAll }) => {
    ...
    const getEntityBadge = (accion: string) => {
        if (accion.includes('ITEM')) return <span className="bg-blue-500/20 text-blue-400 text-[7px] px-1 rounded font-semibold border border-blue-500/20">ÍTEM</span>;
        if (accion.includes('ORDEN')) return <span className="bg-amber-500/20 text-amber-400 text-[7px] px-1 rounded font-semibold border border-amber-500/20">ORDEN</span>;
        if (accion.includes('CUENTA')) return <span className="bg-red-500/20 text-red-400 text-[7px] px-1 rounded font-semibold border border-red-500/20">CUENTA</span>;
        return null;
    };
                ...
                <div className="absolute right-0 mt-2 w-96 bg-[#1e212b] shadow-[0_20px_50px_rgba(0,0,0,0.5)] rounded-2xl border border-white/10 z-[100] overflow-hidden flex flex-col max-h-[600px] animate-in fade-in zoom-in-95 duration-200">
                    <div className="p-5 border-b border-white/5 flex items-center justify-between bg-black/20">
                        <div className="flex flex-col">
                            <h3 className="text-[11px] font-semibold text-white uppercase tracking-[0.2em]">Alertas Críticas</h3>
                            <span className="text-[8px] text-indigo-400 font-medium uppercase tracking-widest mt-0.5">Auditoría en Vivo</span>
                        </div>
                        <span className="text-[10px] font-semibold text-gray-500 uppercase tracking-tighter bg-white/5 px-2 py-0.5 rounded">{notifications.length}</span>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto">
                        {notifications.length === 0 ? (
                            <div className="p-10 text-center py-20">
                                ...
                                <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-widest">Sin alertas críticas hoy</p>
                            </div>
                        ) : (
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex justify-between items-start mb-1.5">
                                                    <div className="flex items-center gap-1.5 truncate">
                                                        ...
                                                        <span className="text-[10px] font-semibold text-white truncate uppercase tracking-wider">
                                                            {n.accion_descripcion}
                                                        </span>
                                                    </div>
                                                    <span className="text-[9px] font-medium text-gray-500 shrink-0 ml-2 bg-black/20 px-1.5 py-0.5 rounded">
                                                ...
                                                <div className="grid grid-cols-2 gap-2 mb-3">
                                                    {(n.metadata?.productName || n.metadata?.product_name || n.metadata?.item_name) && (
                                                        <div className="bg-white/5 p-1.5 rounded border border-white/5">
                                                            <span className="block text-[7px] text-gray-500 uppercase font-semibold tracking-widest leading-none mb-1">Producto</span>
                                                            <span className="text-[9px] font-medium text-gray-300 truncate block uppercase">
                                                                {n.metadata?.productName || n.metadata?.product_name || n.metadata?.item_name}
                                                            </span>
                                                        </div>
                                                    )}
                                                    {n.metadata?.cuenta && (
                                                        <div className="bg-white/5 p-1.5 rounded border border-white/5">
                                                            <span className="block text-[7px] text-gray-500 uppercase font-semibold tracking-widest leading-none mb-1">Cuenta</span>
                                                            <span className="text-[9px] font-medium text-gray-300 truncate block uppercase">
                                                                {n.metadata?.cuenta}
                                                            </span>
                                                        </div>
                                                    ...
                                                    {(n.metadata?.order_number || n.metadata?.numero_orden || n.metadata?.orderId || n.metadata?.order_id) && (
                                                        <div className="bg-white/5 p-1.5 rounded border border-white/5">
                                                            <span className="block text-[7px] text-gray-500 uppercase font-semibold tracking-widest leading-none mb-1">Orden / ID</span>
                                                            <span className="text-[9px] font-medium text-gray-300 block uppercase">
                                                    ...
                                                    {(n.metadata?.table_number || n.metadata?.mesa || n.metadata?.table_id) && (
                                                        <div className="bg-white/5 p-1.5 rounded border border-white/5">
                                                            <span className="block text-[7px] text-gray-500 uppercase font-semibold tracking-widest leading-none mb-1">Mesa</span>
                                                            <span className="text-[9px] font-medium text-gray-300 block uppercase">
                                                                {n.metadata?.table_number || n.metadata?.mesa || n.metadata?.table_id}
                                                            </span>
                                                        </div>
                                                    ...
                                                    {(n.metadata?.amount || n.metadata?.monto_anulado || n.metadata?.total_anulado || n.metadata?.total_cuenta) && (
                                                        <div className="bg-white/5 p-1.5 rounded border border-white/5">
                                                            <span className="block text-[7px] text-gray-500 uppercase font-semibold tracking-widest leading-none mb-1">Monto</span>
                                                ...
                                                {n.metadata?.items && Array.isArray(n.metadata.items) && n.metadata.items.length > 0 && (
                                                    <div className="mb-3 bg-black/20 p-2 rounded border border-white/5">
                                                        <span className="block text-[7px] text-gray-500 uppercase font-semibold tracking-widest leading-none mb-1.5">Detalle de Productos</span>
                                                        <div className="flex flex-col gap-1">
                                                            {n.metadata.items.slice(0, 5).map((item: any, idx: number) => (
                                                                <div key={idx} className="flex justify-between items-center text-[9px] font-medium text-gray-400 border-b border-white/5 pb-0.5 last:border-0">
                                                            ...
                                                            {n.metadata.items.length > 5 && (
                                                                <span className="text-[8px] text-indigo-400 font-semibold mt-1 uppercase">+{n.metadata.items.length - 5} productos más...</span>
                                                            )}
                                                        </div>
                                                    </div>
                                                )}
                                                ...
                                                <div className="flex items-center justify-between mt-2 pt-2 border-t border-white/5">
                                                    <div className="flex items-center gap-2">
                                                        <div className="w-5 h-5 rounded-full bg-red-500/20 flex items-center justify-center text-[9px] font-semibold text-red-400 border border-red-500/20">
                                                            {n.usuario_nombre.charAt(0)}
                                                        </div>
                                                        <div className="flex flex-col">
                                                            <span className="text-[7px] text-gray-500 uppercase font-semibold tracking-widest leading-none mb-0.5">Autorizado Por</span>
                                                            <span className="text-[10px] font-semibold text-gray-300 uppercase leading-none">
                                                                {n.usuario_nombre}
                                                            </span>
                                                        </div>
                                                    </div>
                                                    <div className="flex items-center gap-1 text-[8px] font-semibold text-gray-600 uppercase tracking-widest">
                                                        <span>{n.modulo}</span>
                    ...
                    <div className="p-4 bg-black/40 border-t border-white/5 text-center">
                        <button 
                            className="text-[9px] font-semibold text-indigo-400 uppercase tracking-[0.2em] hover:text-indigo-300 transition-colors flex items-center justify-center gap-2 mx-auto"
                            onClick={() => {
                                setIsOpen(false);
