    ...
    <div className="p-6 animate-fade-in w-full h-full flex flex-col relative bg-[#f8f9fc]">
              ...
              <div key={section.id} className="space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-4 px-2">
                  {sectionTables.map((table) => (
                    <div key={table.id} className="bg-white p-6 rounded-2xl relative group border border-gray-200 hover:border-indigo-500/30 transition-all flex flex-col items-center shadow-sm hover:shadow-md">
                      <div className="absolute top-4 right-4 flex gap-1 transition-opacity">
                        <button onClick={() => handleDelete(table.id)} className="p-2 bg-red-50 hover:bg-red-100 rounded-lg text-red-500 hover:text-red-600 shadow-sm transition-all active:scale-90"><Trash2 size={12} /></button>
                      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 pointer-events-none">
          ...
          <DraggableWindow id="tables-admin-modal" title="Mantenimiento de Mesas">
            ...
            <div className="flex justify-between items-center mb-8">
              <h3 className="text-xl font-black uppercase text-gray-900">Agregar Mesa</h3>
              <button onClick={() => setShowModal(false)} className="text-slate-500 hover:text-slate-500"><X size={24} /></button>
            </div>
            ...
            <div className="space-y-6">
              {!bulkMode ? (
                <>
                  <div>
                    ...
                    <input value={newTable.number} onChange={e => setNewTable({ ...newTable, number: e.target.value })} type="number" className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 font-bold text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all" placeholder="Ej. 1" />
                  </div>
                  <div>
              ...
              >
                {bulkMode ? 'Generar Mesas' : 'Guardar Mesa'}
              </button>
            </div>
            </div>
          </DraggableWindow>
