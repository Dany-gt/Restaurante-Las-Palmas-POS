                ...
                <div className="w-[1100px] bg-[#fdfdfd] border-2 border-[#106ebe]  flex flex-col overflow-hidden animate-in zoom-in-95 duration-150">
                    ...
                    <div className="modal-header bg-[#106ebe] h-9 px-4 flex justify-between items-center cursor-move border-b border-black">
                        <div className="flex items-center gap-2 pointer-events-none">
                            ...
                            <span className="text-white text-[11px] font-semibold uppercase tracking-widest">Liquidación Individualizada de Propinas</span>
                        </div>
                    ...
                    <div className="p-4 flex flex-col gap-4 overflow-y-auto max-h-[92vh]">
                        <div className="bg-[#fff9db] border-2 border-amber-300 p-2.5 flex items-center gap-3">
                            ...
                            <p className="text-[11px] font-semibold text-amber-900 leading-tight uppercase">
                        ...
                        <div className="grid grid-cols-12 gap-5">
                            <div className="col-span-12 lg:col-span-3 flex flex-col gap-4">
                                <div className="bg-white border-2 border-slate-200 p-4 ">
                                    <h3 className="text-[10px] font-semibold uppercase text-slate-800 border-b-2 border-slate-100 pb-2 mb-3">Balance Global</h3>
                                    <div className="space-y-4">
                                        <div className="flex flex-col items-center p-2 bg-slate-50">
                                            <span className="text-[9px] font-semibold text-slate-500 uppercase">Recaudación Bruta</span>
                                            <span className="text-2xl font-semibold text-slate-900 font-mono tracking-tighter">{formatCurr(initialTotalBruta)}</span>
                                        </div>
                                        <div className="p-3 bg-red-50 border-2 border-red-100 flex flex-col items-center">
                                            <span className="text-[9px] font-semibold text-red-700 uppercase">Retención Admin (17%)</span>
                                            <span className="text-xl font-semibold text-red-600 font-mono">{formatCurr(results.totalRetencionAdmin)}</span>
                                        </div>
                                        <div className="p-3 bg-[#106ebe] flex flex-col items-center text-white">
                                            <span className="text-[9px] font-semibold text-slate-400 uppercase">Neto Total a Pagar</span>
                                            <span className="text-xl font-semibold text-emerald-400 font-mono">{formatCurr(results.totalFondoMeseros + results.totalFondoApoyo)}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="col-span-12 lg:col-span-9 flex flex-col gap-5">
                                <div className="bg-white border-2 border-slate-300  flex flex-col">
                                    <div className="bg-[#106ebe] p-2.5 flex items-center gap-2">
                                        ...
                                        <span className="text-[10px] font-semibold uppercase text-white tracking-widest">Detalle Fiscal de Meseros</span>
                                    </div>
                                    <div className="overflow-x-auto">
                                        <table className="w-full text-left">
                                            <thead className="bg-slate-100 border-b-2 border-slate-200 text-[9px] font-semibold text-slate-700 uppercase">
                                                <tr>
                                            ...
                                            <tbody className="divide-y divide-slate-200 text-[11px] font-semibold text-slate-800 uppercase bg-white">
                                                {results.meseros.map((m, i) => (
                                                    <tr key={i} className="hover:bg-blue-50 transition-colors">
                                                        <td className="px-3 py-2 border-r border-slate-100 font-medium">{m.name}</td>
                                                        ...
                                                        <td className="px-2 py-1">
                                                            <input
                                                                type="number"
                                                                className="w-full h-7 bg-white border-2 border-slate-300 outline-none text-red-600 text-center font-mono font-semibold focus:border-red-500"
                                                                value={m.vales || ''}
