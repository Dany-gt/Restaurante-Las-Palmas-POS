/**
 * DOMINIO 4 — UTENSILIOS DE COCINA
 * Sidebar exclusivo de Utensilios.
 * SOLO lee / escribe: utensil_categories
 * NUNCA toca: menu_categories, product_categories,
 *              supply_categories, products
 */
import React, { useState } from 'react';
import { createPortal } from 'react-dom';
import { Edit2, Trash2, X, Save, Loader2, Wrench } from 'lucide-react';
...
// ══════════════════════════════════════════════════
const DOMAIN_TABLE = 'utensil_categories' as const;

interface UtensilCategorySidebarProps {
    selectedId: string | null;
    onSelect: (id: string | null) => void;
}

export const UtensilCategorySidebar: React.FC<UtensilCategorySidebarProps> = ({ selectedId, onSelect }) => {
    const notify = useNotify();
    const { categories, loading, create, update, remove } = useDomainCategories({
        table: DOMAIN_TABLE,   // ← SOLO ESTA TABLA
        orderBy: 'sort_order',
        nameField: 'name',     // utensil_categories usa columna 'name'
    });
            ...
            >
                <div className="bg-[#106ebe] h-[26px] px-2 flex items-center justify-between border-b border-[#004578] shrink-0">
                    <div className="flex items-center gap-1.5">
                        <Wrench size={10} className="text-white/80" />
                        <span className="text-[9px] font-medium text-white uppercase tracking-tight">Utensilios</span>
                    </div>
                    <button onClick={() => setForm({ id: null, nombre: '' })}
                        ...
                        title="Nueva categoría de utensilios">+</button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto">
                    ...
                    >
                        <span className="uppercase text-[10px]">— Todos los Utensilios —</span>
                    </div>
                ...
                <div className="h-5 bg-[#f0f0f0] border-t border-gray-300 px-2 flex items-center shrink-0">
                    <span className="text-[8px] font-medium text-gray-400 italic">Módulo: Utensilios de Cocina</span>
                </div>
            </div>
                    ...
                    <div className="bg-[#f0f0f0] border border-[#106ebe] shadow-2xl w-[320px] overflow-hidden">
                        <div className="bg-[#106ebe] h-8 px-3 flex items-center justify-between text-white">
                            <div className="flex items-center gap-2">
                                <Wrench size={13} />
                                <span className="text-[11px] font-medium uppercase">{form.id ? 'Editar' : 'Nueva'} Cat. de Utensilios</span>
                            </div>
