...
import { Search, RotateCcw, Loader2, CheckSquare, ArrowUpDown, Filter, Plus, Edit3, Trash2, Save, X } from 'lucide-react';
...
export const DishesOptionsList: React.FC = () => {
    ...
    const fetchGroups = async () => {
        const { data } = await supabase.from('option_groups').select('*').order('name');
    ...
    const handleDelete = async (item: any) => {
        setConfirmAction({
            message: `¿Está seguro que desea eliminar la opción "${item.item_name}"?`,
            onConfirm: async () => {
                const { error } = await supabase.from('group_items').delete().eq('id', item.id);
                if (error) {
                    notify.error('Error al eliminar opción: ' + error.message);
                } else {
                    fetchData();
                }
    ...
    const handleSave = async () => {
        ...
        if (duplicateError) {
            ...
            if (isDup) {
                notify.alert('Esta opción ya existe en el grupo seleccionado.');
                setSaving(false);
                return;
            }
        }
    ...
    const filteredItems = sortedItems.filter(item =>
        ...
        item.option_groups?.name.toLowerCase().includes(searchQuery.toLowerCase())
    );
